...
#### Function: Sequence Quality Check and Filter Based on Gaps and "N" Content ----
...
quality_check_and_filter_sequences <- function(df) {
  ...
  df <- df %>%
    filter(
      !str_detect(sequence, "-") &                   # Remove sequences with gaps
        !str_detect(sequence, "^N") &                  # Remove sequences starting with 'N'
        !str_detect(sequence, "N$") &                  # Remove sequences ending with 'N'
        str_count(sequence, "N") < (0.05 * str_length(sequence))  # Remove sequences with high 'N' content
    )
